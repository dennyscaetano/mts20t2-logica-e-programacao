...
const { somarDoisNumeros, subtrairDoisNumeros, multiplicarDoisNumeros, dividirDoisNumeros, calcularMediaDeDoisNumeros } = require('../src/calculadora');

describe('Testes de Operações da Calculadora', () => {
    // Testes para a função somarDoisNumeros
    ...
    describe('Testes da Função de Cálculo de Média', () => {
        it('A função deve ser capaz de calcular a média de dois números positivos', () => {
            ...
            if (resultadoDaMedia !== 5) throw new Error(`Esperado 5, mas recebeu ${resultadoDaMedia}`);
        });
        it('A função deve ser capaz de calcular a média de um número positivo e um negativo', () => {
            const resultadoDaMedia = calcularMediaDeDoisNumeros(6, -4);
            expect(resultadoDaMedia).to.equal(1);
            if (resultadoDaMedia !== 1) throw new Error(`Esperado 1, mas recebeu ${resultadoDaMedia}`);
        });
        ...
        it('A função deve ser capaz de calcular a média quando um dos números é zero', () => {
            ...
            if (resultadoDaMedia !== 3) throw new Error(`Esperado 3, mas recebeu ${resultadoDaMedia}`);
        });
        it('A função deve ser capaz de calcular a média de dois zeros', () => {
            ...
            if (resultadoDaMedia !== 0) throw new Error(`Esperado 0, mas recebeu ${resultadoDaMedia}`);
        });
    });
});